...
\textcolor{blue}{We can also use our framework to understand trajectory anomalies, offering developers useful signal for when their agents are failing. The traces show \emph{why} the scaffolds diverge on these tasks. On the 63 instances where Agentless ``wins'' (successfully passing a task that SWE-agent did not), SWE-agent's failing runs are long and edit-heavy: they average 33 steps (vs.\ Agentless's 13) and are over twice as likely to contain a streak of five or more consecutive edits (30\% vs.\ 13\% of SWE-agent's own successful runs). Agentless instead localizes the error, patches the problem, and validates in a tight ${\sim}13$-step loop. The failure signature here is edit-thrashing and length, not browsing.}

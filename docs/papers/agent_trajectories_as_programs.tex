...
\textcolor{blue}{\textbf{Contributions.} (1) \texttt{procgrep}, a library that canonicalizes agent traces into a bottom-up procedural vocabulary and supports fingerprinting, divergence, procedural reward, and lineage diffing. (2) Procedural fingerprints identify the producing agent at 85.7\% accuracy, leakage-controlled by task---process-level attribution, complementary to output stylometry. (3) Structure beats self-report: agents narrate only part of what they do, and structural representations predict outcomes better than natural-language accounts. (4) Operational use---an early-abort/cost-control policy grounded in procedural signatures. (5) A distilled-model case study separating what output-only fine-tuning transfers (the action vocabulary) from what it does not (it concentrates behavior into rigidity).}
...
$H(\text{actions} \mid \text{procedure})$ measures the confidence of belonging of an action to a procedure, and $H(\text{procedure} \mid \text{actions})$ measures the reverse. A vocabulary is stable when $V$ is near-maximal---in practice a broad plateau rather than a single optimum (Figure~\ref{fig:vmeasure}).
...
\textcolor{blue}{Applied across all ten agents (Figure~\ref{fig:jsd_full}), the distilled child's nearest neighbour is its teacher (JSD 0.25), whereas changing the scaffold or the model generation moves behavior about twice as far (${\approx}0.53$)---distillation preserves procedure more than either.}

Across all models, passing behavior includes a higher ratio of browsing to other actions. Extended thinking Claude models spend the largest proportion of their time in the shell compared to older models with more even task breakdowns. Models in the GPT family have more consistent behavior, but newer ones spend more time editing. Agent harnesses have less task diversity, potentially due to their programmed behavior and have more distinct breakdowns, the Agentless scaffold spends a disproportionate amount of time browsing and the Moatless scaffold leads to a large proportion of testing. An interesting application of this is forecasting the costs of an agent, as it is not only a product of the cost per tool call, but a function of the action types needed to complete it and the procedural tendencies of the model being orchestrated. \textcolor{blue}{Figures~\ref{fig:step_token_cost}, \ref{fig:steps_resolved}, and~\ref{fig:token_cost_difficulty} break this down: cost by action type and tokens, steps per resolved task, and the monotonic decline of tokens, API calls, and dollar cost as task difficulty falls.}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/fig_step_vs_token_cost.png}
    \caption{\textcolor{blue}{Per-agent cost decomposed by action type and by token volume. Because per-step cost is dominated by inference calls, agents that route work through cheaper shell/orchestration steps before invoking the model spend less per step (cf.\ the \$/step column of Table~\ref{tab:per-agent-megatable}).}}
...
\begin{figure}[H]
    ...
    \caption{\textcolor{blue}{Mean trajectory length on resolved tasks, per agent. Deterministic and constrained scaffolds (Agentless, Moatless: ${\sim}13$ steps) resolve in a fraction of the steps that open-ended model-driven SWE-agent runs take (e.g.\ Claude-4 at 64.8), a length difference that drives the cost-per-resolved gap in Table~\ref{tab:per-agent-megatable}.}}
...
Additionally, we look at different agent pipelines and compare them across 289 tasks (Agentless v.\ SWE-agent). Agentless successfully solves 63 more tasks than SWE-agent and we can use the procedural methods for assessing why.
...
\begin{figure}[H]
    ...
    \caption{\textcolor{blue}{Pairwise procedural JSD (base-2 Jensen--Shannon distance over per-trajectory canonical distributions) across ten agents. The distilled child (SWE-agent-LM-32B) and its teacher (Claude-3.7~Sonnet) form the tightest model pair (JSD 0.25, the child's nearest neighbour by a factor of two). Changing the scaffold (Claude-3.5 under Agentless vs.\ SWE-agent, 0.53) or the model generation (within-family, mean 0.52) moves behavior about twice as far---so lineage $\ll$ scaffold $\approx$ era. The older dense models converge across families (Claude-3 vs.\ GPT-4o, 0.20, the closest pair overall).}}
    \label{fig:jsd_full}
\end{figure}
...
\subsection{\textcolor{blue}{Classifier prompt}}
\textcolor{blue}{The four judges were prompted with the following reference-grounded rubric; the \%-compositional and $\kappa$ in Table~\ref{tab:divergence_by_model} are computed over its \texttt{divergence\_level} output.}
\begin{tcolorbox}[colback=black!3, colframe=black!30, boxrule=0.4pt, left=10pt, right=10pt, top=8pt, bottom=8pt, fontupper=\small]
\textcolor{blue}{
\textbf{Judge prompt (divergence classifier).}\\[2pt]
You are evaluating a software engineer's response to a bug report. You are given the reference patch---the actual fix---and use it as ground truth.\\[3pt]
\texttt{Bug report: \{problem\_statement\}}\\
\texttt{Reference patch: \{gold\_patch\}}\\
\texttt{Response being evaluated: \{response\}}\\[3pt]
Reason briefly about each dimension, then score it $0$--$3$ ($0$ = wrong, $3$ = exact match to the reference): \emph{localization} (same file/function as the patch?), \emph{edit\_type} (right kind of change?), \emph{plan\_quality} (would this plan reproduce the patch?), \emph{explanation} (grounded in what the patch does?).\\[3pt]
Identify the first plan step where the response diverges ($-1$ if fully aligned) and its level:\\
\quad\emph{surface} --- wrong file/function name (token-level mismatch)\\
\quad\emph{compositional} --- right location, wrong operation or order (syntactic-level)\\
\quad\emph{relational} --- right operations, wrong dependency or interaction (graph-level)\\
\quad\emph{none} --- no divergence\\[3pt]
Output per-dimension reasoning, then:\\
\texttt{\{"localization":int, "edit\_type":int, "plan\_quality":int,}\\
\texttt{~~"explanation":int, "first\_deviation\_step":int,}\\
\texttt{~~"divergence\_level":"surface|compositional|relational|none"\}}
}
\end{tcolorbox}
...
\textcolor{blue}{\textbf{What moves procedure most.} Decomposing the JSD matrix (Figure~\ref{fig:jsd_full}) by what is varied: \emph{distillation} preserves procedure---the SWE-agent-LM-32B child diverges from its Claude-3.7 teacher by only 0.25, its nearest neighbour by a factor of two. Changing the \emph{scaffold} (Claude-3.5 under Agentless vs.\ SWE-agent, 0.53) or the \emph{model generation} within a family (mean 0.52, range 0.27--0.77) moves behavior roughly twice as far, and the two are comparable---so lineage $\ll$ scaffold $\approx$ era, rather than a strictly scaffold-dominant ordering. The extremes combine both axes (Agentless+Claude-3.5 vs.\ Claude-4, 0.90).}
...
\subsection{\textcolor{blue}{Behavioral follow-through and verbosity}}
\textcolor{blue}{Forward follow-through (does the stated plan cover the actions taken) and reverse follow-through (does the account mention each action taken) are operationalized as set coverage: we align the stated action set against the observed canonical action set per trajectory (this measures whether actions are accounted for, not whether each stated intent is executed in order). Verbosity is the ratio of stated to observed actions, with a shuffled-rationale control as baseline (Table~\ref{tab:verbosity}, Figure~\ref{fig:cot_alignment}).}

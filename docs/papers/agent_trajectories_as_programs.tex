...
\textcolor{blue}{The traces show \emph{why} the scaffolds diverge on these tasks. On the 63 instances Agentless solves but SWE-agent does not, SWE-agent's failing runs are long and edit-heavy: they average 33 steps (vs.\ Agentless's 13) and are over twice as likely to contain a streak of five or more consecutive edits (30\% vs.\ 13\% of SWE-agent's own successful runs). Agentless instead localizes, patches, and validates in a tight ${\sim}13$-step loop. The failure signature here is edit-thrashing and length, not browsing.}

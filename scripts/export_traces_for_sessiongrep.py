...
2. docs/distillation_run/child_traj/*.traj — 499 SWE-agent-LM-32B rollout
   trajectories (HF midah/bidirect-distillation-traces; fetch per
   docs/distillation_run/MOVED.md) -> bidirect-rollout-<instance_id>
...
ROLLOUTS = REPO / "docs" / "distillation_run" / "child_traj"
...
def export_rollouts() -> int:
    if not ROLLOUTS.is_dir():
        print(f"rollouts not present at {ROLLOUTS} — see docs/distillation_run/MOVED.md; skipping")
        return 0
    written = 0

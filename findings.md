...
- **`tab:divergence_by_model`: only `n` is verifiable** (286/285/87/87 match `output/prompting_study/*/summary.json`). F1(k=1), %compositional, and mean κ are **not persisted anywhere** (records hold only raw judge text; parsed labels + κ never saved) — same untraceability as the κ-flag above. Re-derive + persist, or caveat.

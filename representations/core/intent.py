...
class EmergentIntentTaxonomy:
    ...
    def extract_intent_description(
        ...
        event: dict,
        use_llm: bool = True,
        llm_extractor: Callable[..., str] | None = None,
    ) -> str:
        """
...
Your response:"""
        ...
        except (OSError, KeyError, IndexError, ValueError, AttributeError):
            return self._heuristic_intent_description(
                event.get("type", ""),
    ...
    def discover_taxonomy(
        ...
        for cluster_id in unique_labels:
            ...
            cluster_descriptions = [d for d, m in zip(descriptions, mask, strict=True) if m]
...
Label:"""
    ...
    def save_taxonomy(self, path: Path | None = None) -> Path:
    ...
    def load(self, path: Path | None = None) -> bool:
        ...
        try:
            ...
            return True
        except (OSError, KeyError, pickle.UnpicklingError, EOFError, TypeError):
            return False
...
_emergent_taxonomy: EmergentIntentTaxonomy | None = None
...
def extract_intent_vector(text: str) -> list[str]:
    ...
    if any(
        word in text_lower
        for word in ["fix", "error", "bug", "debug", "broken", "issue", "problem", "wrong", "crash", "exception"]
    ):
    ...
    if any(
        word in text_lower
        for word in ["add", "create", "implement", "new", "build", "make", "feature", "functionality"]
    ):
    ...
    if any(
        word in text_lower
        for word in ["refactor", "clean", "improve", "optimize", "restructure", "reorganize", "simplify"]
    ):
    ...
    if any(
        word in text_lower for word in ["document", "comment", "explain", "describe", "docstring", "readme", "docs"]
    ):
        intents.append("INTENT_DOCUMENT")

    # Code review/explanation intent
    if any(
        word in text_lower for word in ["review", "explain", "understand", "what", "how", "why", "analyze", "inspect"]
    ):
...
def extract_prompt_from_event(event: dict) -> str | None:
    """Extract prompt text from an event. Handles various event structures."""
    if event is None or not isinstance(event, dict):
        return None
    event_type_raw = event.get("type")
    event_type = str(event_type_raw).lower() if event_type_raw else ""
    if event_type in ("prompt", "prompt_sent", "conversation", "ai_prompt"):
        text = event.get("text") or event.get("content") or event.get("prompt") or event.get("message")
        if text:
            return str(text)
        details = event.get("details", {})
        if isinstance(details, dict):
            text = details.get("text") or details.get("content") or details.get("prompt") or details.get("message")
            if text:
                return str(text)
        elif isinstance(details, str):
            try:
                details_dict = json.loads(details)
                text = details_dict.get("text") or details_dict.get("content") or details_dict.get("prompt")
                if text:
                    return str(text)
...
def extract_keyword_intent(event: dict) -> str:
    """Extract single keyword-based intent category from an event."""
    intents = extract_event_intent(event)
    return intents[0] if intents else "INTENT_OTHER"


def extract_event_intent(event: dict) -> list[str]:
    ...
    if file_path:
        ...
        if any(p in path_str for p in ["test", "spec", "__tests__", ".test.", ".spec."]):
            intents.append("INTENT_TEST")
        if any(
            p in path_str
            for p in ["config", "settings", ".env", "package.json", "requirements.txt", "setup.py", "pyproject.toml"]
        ):
            intents.append("INTENT_CONFIGURE")
        if any(p in path_str for p in ["readme", "docs", ".md", "docstring", "changelog"]):
            intents.append("INTENT_DOCUMENT")
    ...
    if diff_summary_lower:
        ...
        # Feature signals
        if any(
            word in diff_summary_lower
            for word in ["add", "create", "implement", "new", "feature", "function", "method", "class"]
        ):
            intents.append("INTENT_FEATURE")
        # Refactor signals
        if any(
...
def intent_tokens_for_event(
    ...
    """Extract intent tokens from an event."""
    ...
    intent_vector = extract_event_intent(event)
    tokens.extend(intent_vector)
...
def compare_intent_methods(
    ...
    for event in sample:
        ...
        if has_taxonomy:
            ...
            # Check for agreement
            keyword_set = {k.replace("INTENT_", "").lower() for k in keyword}
            emergent_labels = [e.replace("EMERGENT_INTENT_", "").lower() for e in emergent]

            # Simple heuristic: any word overlap?
            overlap = any(any(kw in el or el in kw for kw in keyword_set) for el in emergent_labels)

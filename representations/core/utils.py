...
import extraction, and file operations.
"""
...
def _extract_code_tokens(
    ...
    Uses AST parsers for enabled languages, falling back to generic tokenization
    if parsing fails or the language parser is not enabled.
    ...
    if language == "python" and "python" in enabled:
        try:
            return _tokenize_python_ast(code)
        except SyntaxError:
            pass

    if language in ("javascript", "typescript") and "javascript" in enabled:
        try:
            return _tokenize_js_ast(code, language or "javascript")
        except (ImportError, SyntaxError, ValueError, TypeError):
            pass

    if language == "java" and "java" in enabled:
        try:
...
def dependencies_repr(trace: dict) -> str:
    ...
    for event in trace.get("events", []):
        ...
        file_path = details.get("file_path") or details.get("file") or details.get("target")
        if not file_path:
            continue

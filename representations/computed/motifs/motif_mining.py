...
class MotifRegistry:
    ...
    @classmethod
    def get_original(cls, hashed: str) -> str | None:
...
def extract_universal_motifs(
    ...
    ngram_sizes = ngram_sizes or [3, 4]
    motifs = []
...
def extract_structural_motifs(trace: dict) -> list[str]:
    ...
    if len(file_switches) >= 2:
        switch_pairs = [(file_switches[i][1], file_switches[i + 1][1]) for i in range(len(file_switches) - 1)]
